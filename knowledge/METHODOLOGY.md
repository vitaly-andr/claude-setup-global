# Методика Memory Bank

Система накопления и использования проверенных знаний для агентов Claude Code.

## Философия

**Цель**: Минимизировать повторные обращения к внешним источникам за информацией, которую мы уже успешно применяли.

**Принцип**: Каждое успешное решение → новое знание в базе

**Качество**: Только проверенные, протестированные решения

## Рабочий процесс

### 1. Решение задачи (sysadmin)

При получении задачи sysadmin агент:

1. **Определяет контекст** - проект или глобальный
   ```bash
   ls .claude/ 2>/dev/null && echo "Project" || echo "Global"
   ```

2. **Проверяет базы знаний по приоритету**:
   - **Сначала проектную** (если в проекте): `./.claude/knowledge/`
   - **Затем глобальную**: `~/.claude/knowledge/sysadmin/`
   - Читает релевантные файлы из категорий
   - Ищет похожие решенные проблемы

3. **Если решения нет локально** → использует WebSearch/WebFetch
   - Ищет в официальной документации
   - Проверяет Arch Wiki, GitHub issues, официальные форумы

4. **Применяет решение** и тестирует

5. **Если решение успешно** → определяет КУДА сохранить → переходит к этапу сохранения

### 2. Валидация решения (librarian)

После успешного решения sysadmin вызывает librarian для проверки:

```
"Прошу librarian проверить корректность примененного решения
и подтвердить его для сохранения в memory bank"
```

Librarian проверяет:
- ✅ Решение корректно по документации
- ✅ Использованы актуальные версии и команды
- ✅ Нет deprecated методов или уязвимостей
- ✅ Решение переиспользуемо для похожих ситуаций

### 3. Определение места сохранения

**КРИТИЧЕСКИ ВАЖНО**: Перед сохранением определить контекст решения!

**Decision Tree**:
```
Решение найдено и работает
    ↓
Это решение для...?
    ↓
┌─────────────────────────┬──────────────────────────┐
│ ПРОЕКТА                 │ СИСТЕМЫ                  │
│ (project-specific)      │ (system-wide)            │
├─────────────────────────┼──────────────────────────┤
│ • Docker setup          │ • pacman команды         │
│ • Dependencies          │ • Hyprland config        │
│ • Django migrations     │ • Waybar setup           │
│ • API endpoints         │ • systemd services       │
│ • Project env vars      │ • Kernel modules         │
│ • Build scripts         │ • Arch Linux общее       │
│ • Deployment steps      │ • Desktop environment    │
│ • Framework config      │ • Network config         │
└─────────────────────────┴──────────────────────────┘
         ↓                           ↓
  ./.claude/knowledge/        ~/.claude/knowledge/
   (версионируется             (синхронизируется
    с проектом)                 между машинами)
```

**Если неочевидно** → Спросить пользователя:
```
"Это решение специфично для этого проекта или может
использоваться системно на всех проектах?"

1. Проект (./.claude/knowledge/)
2. Система (~/.claude/knowledge/)
```

### 4. Сохранение в Memory Bank

**Формат файла решения**:
```markdown
# [Четкое описание проблемы]

> Дата: YYYY-MM-DD
> Валидировано: librarian
> Система: [версия ОС/ПО]
> Теги: #tag1 #tag2 #tag3

## Контекст
Краткое описание ситуации, когда применимо

## Решение
\`\`\`bash
# Конкретные команды с комментариями
\`\`\`

## Объяснение
Почему это работает, важные детали

## Альтернативы
Другие подходы, если применимо

## Источники
- [Документация](url)
- [Проверено](url)
```

**Куда сохранять**:

**Для СИСТЕМНЫХ решений** (`~/.claude/knowledge/`):

1. **Краткосрочное** → `~/.claude/knowledge/sysadmin/solutions/YYYY-MM-DD-описание.md`

2. **Долгосрочное** → категория:
   - `arch-linux/` - команды pacman, системные настройки
   - `hyprland/` - конфигурации Hyprland
   - `waybar/` - конфигурации Waybar
   - `common-issues/` - частые проблемы и их решения

**Для ПРОЕКТНЫХ решений** (`./.claude/knowledge/`):

1. **Краткосрочное** → `./.claude/knowledge/solutions/YYYY-MM-DD-описание.md`

2. **Долгосрочное** → категория:
   - `setup/` - настройка проекта
   - `docker/` - Docker конфигурации
   - `deployment/` - деплой инструкции
   - `issues/` - частые проблемы проекта
   - ИЛИ создать новую категорию под нужды проекта

### 5. Обновление базы знаний

**Когда обновлять категории**:
- При накоплении 3+ похожих решений → обобщить в категорию
- При обнаружении устаревшего решения → обновить с пометкой даты
- При нахождении более эффективного метода → добавить секцию "Обновления"

**Workflow обновления**:

```
sysadmin: "Обнаружил, что решение X устарело, нашел новое Y"
↓
librarian: "Подтверждаю, решение Y актуально для версии Z"
↓
sysadmin: "Обновляю файл в knowledge base с пометкой устаревания X"
```

## Шаблоны

### Новая команда/решение

```markdown
### [Название]
\`\`\`bash
команда
\`\`\`
**Когда использовать**: описание
**Версия**: X.Y.Z+
**Проверено**: YYYY-MM-DD
```

### Частая проблема

```markdown
# [Проблема]

**Симптомы**: что видим
**Причина**: почему происходит
**Решение**:
\`\`\`bash
команды
\`\`\`
**Проверено**: YYYY-MM-DD на [система версия]
```

## Метрики эффективности

Отслеживаем (неформально):
- Сколько раз использовали локальное решение vs новый поиск
- Сколько решений добавлено в базу
- Сколько раз обновляли устаревшие решения

**Цель**: Через месяц использования >70% задач решаются из локальной базы

## Правила использования

### Для sysadmin

1. **ВСЕГДА** сначала проверяй `~/.claude/knowledge/sysadmin/`
2. **ПРЕДЛАГАЙ** сохранение после каждого нетривиального успешного решения
3. **ОБРАЩАЙСЯ** к librarian для валидации перед сохранением
4. **ОБНОВЛЯЙ** базу при обнаружении устаревших методов

### Для librarian

1. **ПРОВЕРЯЙ** решения на актуальность и корректность
2. **ОТМЕЧАЙ** источники и даты валидации
3. **ПРЕДУПРЕЖДАЙ** об устаревших методах
4. **МОЖЕТ РЕДАКТИРОВАТЬ** только documentation файлы в knowledge/

### Для пользователя

1. **Глобальная база** (`~/.claude/knowledge/`):
   - Синхронизируется через git между вашими машинами
   - Периодически просматривай `solutions/` для выявления паттернов
   - Помогай объединять похожие решения в категории

2. **Проектная база** (`./.claude/knowledge/`):
   - Хранится в репозитории проекта
   - Доступна всей команде через `git pull`
   - Коммитьте вместе с кодом
   - Удаляй устаревшие решения при рефакторинге

## Примеры использования

### Пример 1: Системное решение (сохраняется глобально)

```
Пользователь: "Настрой multi-monitor setup для Hyprland"
↓
sysadmin:
  1. Определяет контекст: работаем в /home/user (не проект)
  2. Читает ~/.claude/knowledge/sysadmin/hyprland/basic-config.md
  3. Находит похожую конфигурацию
  4. Применяет с адаптацией
  5. Работает!
  6. Это СИСТЕМНОЕ решение (Hyprland конфиг)
  7. Вызывает librarian: "Validate for GLOBAL knowledge"
  8. Librarian сохраняет → ~/.claude/knowledge/sysadmin/hyprland/
```

### Пример 2: Проектное решение (сохраняется локально)

```
Пользователь в Django проекте: "Настрой Docker для development"
↓
sysadmin:
  1. Определяет контекст: в /projects/my-django-app (есть .git и .claude)
  2. Читает ./.claude/knowledge/docker/ - пусто
  3. Читает ~/.claude/knowledge/ - нет Django+Docker
  4. WebSearch → находит best practices
  5. Создает Dockerfile и docker-compose.yml
  6. Работает!
  7. Это ПРОЕКТНОЕ решение (специфично для этого Django app)
  8. Вызывает librarian: "Validate for PROJECT knowledge"
  9. Librarian сохраняет → ./.claude/knowledge/docker/development-setup.md
 10. Решение теперь в git репозитории проекта, доступно команде
```

### Пример 3: Смешанный случай (спросить пользователя)

```
Пользователь в проекте: "Настрой nginx для reverse proxy"
↓
sysadmin:
  1. Определяет контекст: в проекте
  2. Читает ./.claude/knowledge/ и ~/.claude/knowledge/
  3. WebSearch → находит конфигурацию nginx
  4. Применяет → работает!
  5. НЕ УВЕРЕН: это общий nginx или специфично для проекта?
  6. Спрашивает: "Это решение:
     1. Специфично для проекта (сохранить в ./.claude/knowledge/)
     2. Общее для nginx (сохранить в ~/.claude/knowledge/)"
  7. Пользователь: "2 - общее"
  8. Вызывает librarian: "Validate for GLOBAL"
  9. Librarian сохраняет → ~/.claude/knowledge/sysadmin/[nginx category]
```

### Пример 4: Устаревшее решение

```
sysadmin пытается применить решение из knowledge/
↓
Решение не работает (пакет переименован/удален)
↓
WebSearch → находит новое решение
↓
librarian проверяет
↓
sysadmin обновляет файл в knowledge/:
  - Помечает старое решение как "⚠️ Устарело с версии X"
  - Добавляет новое решение
  - Указывает дату обновления
```

## Структура файлов знаний

### Категоризация

**arch-linux/** - Системные команды и настройки:
- package-management.md
- system-services.md
- kernel-modules.md
- networking.md

**hyprland/** - Специфика Hyprland:
- monitors.md
- keybindings.md
- window-rules.md
- animations.md

**waybar/** - Конфигурация Waybar:
- modules.md
- styling.md
- custom-modules.md

**common-issues/** - Проблемы и решения:
- По одному файлу на проблему
- Индексируются в README.md

**solutions/** - Хронологический архив:
- YYYY-MM-DD-description.md
- Позволяет отследить эволюцию решений

## Обслуживание базы

### Еженедельно
- Просмотр новых solutions/
- Выявление паттернов для категоризации

### Ежемесячно
- Проверка актуальности решений
- Обновление версий в документации
- Чистка дубликатов

### По необходимости
- Реорганизация структуры при росте базы
- Создание новых категорий

---

## Быстрый старт

1. **Создана структура** в `~/.claude/knowledge/`
2. **Настроены агенты** sysadmin и librarian
3. **Начинаем работу**:
   - Решаем задачи как обычно
   - Сохраняем успешные решения
   - База растет органически

**Помни**: Качество > Количество. Лучше 10 проверенных решений, чем 100 непроверенных.

---
Создано: 2025-10-21
Версия: 1.0
